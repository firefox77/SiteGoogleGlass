...
class CMB2_Fields implements Runner {
	...
	public function render_advanced_robots( $field, $escaped_value, $object_id, $object_type, $field_type_object ) {
		...
		$strings = [
			'max-snippet'       => __( 'Max Snippet', 'rank-math' ),
			'max-video-preview' => __( 'Max Video Preview', 'rank-math' ),
			'max-image-preview' => __( 'Max Image Preview', 'rank-math' ),
		];
		...
		foreach ( $values as $id => $value ) :
			...
			echo '<li>';
				...
				echo esc_html( $field->get_string( $id . '_text', $strings[ $id ] ) ) . '</label>';
