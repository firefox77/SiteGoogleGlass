		...
		class tvr_microthemer_admin {
				...
				foreach ($this->propertyoptions as $prop_group => $array){
					foreach ($array as $prop => $meta) {
						...
						if ( !empty($meta['sug_values']) ){
							...
							$copiedSrc = ($sug_by_default and !empty($meta['select_options']))
                                ? $meta['select_options']
                                : array();
			...
			*/
			...
			function all_option_groups_html($section_name, $css_selector, $array){
				...
				$html.= $this->mt_hor_scroll_buttons('style', 'li') .
		...
		class tvr_microthemer_frontend {
			...
			*/
			...
			function add_meta_tag() {
				if ( is_user_logged_in() ) {
					?><meta name='iframe-url' id='iframe-url'
                            content='<?php echo rawurlencode(tvr_common::strip_page_builder_and_preview_params($this->currentPageURL()));?>' />
